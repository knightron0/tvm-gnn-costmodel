...
def main(args):
    ...
    try:
        ...
        for input in tqdm(inputs, total=len(inputs)):
            metadata = input.task.workload_key
            all_data.append(metadata)
            i += 1
    
        with open(f"/scratch/gilbreth/mangla/metadata_gnn/{args.filename.replace('.json', '')}.json", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pattern", type=str, help="Pattern to match JSON files", default="")
